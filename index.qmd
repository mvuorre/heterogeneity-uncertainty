...
fig_1_p_rope <- fig_1_descriptors +
  stat_slab(
...
(fig_1_params | fig_1_hi | fig_1_p_neg | fig_1_p_rope) &
  scale_y_continuous(
...
#| fig-width: 2.7
#| fig-height: 2.2
#| fig-cap: "Construction of point estimates of the limits of the heterogeneity interval. To construct a $\\pi%$ heterogeneity interval, $HI_\\pi$, we use the inverse cumulative distribution function (CDF) of the heterogeneity distribution, $\\Phi^{-1}$. When $\\pi = 0.90$, we have $HI_90 = \\Phi^{-1}\\left((1 \\pm \\pi)/2 | \\beta_1, \\tau_1 \\right) = \\Phi^{-1}\\left([0.05, 0.95] | \\beta_1, \\tau_1 \\right)$."
...
```{r}
#| label: fig-p-neg-point
#| fig-width: 2.7
#| fig-height: 2.2
#| fig-cap: "Construction of the point estimate of $p^{-1}$, the proportion of individuals in the population with negative effects. To construct $p^{-1}$, we pass 0 to the cumulative distribution function (CDF) of the heterogeneity distribution."

# construction of p^- on the CDF
p_ml_cdf_p_neg <- p_ml_cdf +
  geom_segment(
...
#| fig-width: 5.4
#| fig-height: 2.2
#| fig-cap: "Bayesian estimates of the heterogeneity distribution of valence effects. A: Probability density function (PDF) curves. The thick line is the same expected PDF of valence effects from Figure 1. Thin lines show 100 PDFs calculated from random draws of $\\beta_1$ and $\\tau_1$ that collectively illustrate the uncertainty in the distribution's location and spread. B: Cumulative distribution function (CDF) curves, annotated as in A. The blue lines show the calculation of the point estimate of $p^- = Pr(\\textrm{Valence effect} \\le 0)$, the proportion of individuals with negative valence effects on logRT, using the mean CDF of valence effects."
...
```{r}
#| label: fig-hi-samples
#| fig-width: 2.7
#| fig-height: 2.2
#| fig-cap: "Bayesian derivation of the posterior distribution for the lower and upper limits of the heterogeneity interval. Thin gray lines show 100 CDFs of the distribution of valence effects calculated from random draws of $\\beta_1$ and $\\tau_1$, as in @fig-2 B. Blue lines show the calculation of the posterior distribution of $HI_{90} = \\left[\\Phi^{-1}(0.05 | \\beta_1, \\tau_1), \\Phi^{-1}(0.95 | \\beta_1, \\tau_1)\\right]$, which is also depicted as marginal histograms. Compare to the point estimate of the same interval endpoints in @fig-hi-point."
...
```{r}
#| label: fig-rope-densities
#| fig-width: 5.4
#| fig-height: 2.8
#| fig-cap: "Bayesian derivation of the posterior distribution for $p^{ROPE}$."
...
p_rope_densities <- post1_samples_rope |> 
  ...
  scale_fill_manual(values = c("TRUE" = desc_color, "FALSE" = alpha(valence_dist_samples_color, 0.3))) +
  guides(fill = "none") +
...
p_rope_props <- post1_samples_rope |> 
  ...
  theme(axis.ticks.y = element_blank()) +
  ylab(NULL) +
  xlab(quote(italic(p)^italic(ROPE))) +
  stat_histinterval(
...
p_rope_densities + p_rope_props + plot_annotation(tag_levels = "A")
```
...
#| fig-width: 2.7
#| fig-height: 2.2
#| fig-cap: "Bayesian derivation of the posterior distribution of $p^- = Pr(\\textrm{Valence effect} \\le 0)$, the proportion of individuals with negative valence effects on logRT. Thin gray lines show 100 CDFs of the distribution of valence effects calculated from random draws of $\\beta_1$ and $\\tau_1$, as in @fig-2 B. Blue lines show the calculation of the posterior distribution of $p^-$, which is also depicted as a marginal histogram at the top left. The point estimate and interval represents the posterior mean and 95\\%CI. Compare to the point estimate of $p^-$ in @fig-2 B."
