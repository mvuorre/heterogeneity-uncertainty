...
# Packages (rmarkdown & knitr required by renv)
library(rmarkdown)
library(knitr)
library(janitor)
library(scales)
...
(We recognize that there are better alternatives to modeling the log-transformed RTs as normal, but those are outside the scope of this manuscript. Interested readers can consult existing literature on reaction time models [@luceResponseTimesTheir1991; @ratcliffDiffusionDecisionModel2008] and their hierarchical extensions [@vandekerckhoveHierarchicalDiffusionModels2011].) Then, we specified a model of the mean of the logRT distribution ($\eta_{ij}$) such that the regression coefficients captured our substantive questions:

$$
\eta_{ij} = \beta_0 + \gamma_{0j} + \left(\beta_1 + \gamma_{1j}\right)\text{V}_{ij}.
$$ {#eq-m1-2}

This equation includes two sets of parameters: The first set contains $\beta_0$ (*beta*), the intercept, and $\beta_1$, the slope or effect of valence (V). Parameters in this set do not have further subscripts: In the frequentist tradition, they are considered constants---not modelled on covariates and having a single true value---and typically referred to as "fixed" parameters [e.g. @raudenbushHierarchicalLinearModels2002]. The second set of parameters, $\gamma_{0j}$ (*gamma*) and $\gamma_{1j}$, have the subscript $j$ to indicate that they are person-specific deviations from the average intercept and slope, respectively. That is, $\beta_0 + \gamma_{01}$ is the intercept (average log reaction time when V=0) for person $j=1$. In frequentist texts, these are typically called "random" parameters because they are modeled as varying randomly according to a specified distribution. Following standard multilevel modeling assumptions, we model $\gamma_0$ and $\gamma_1$ as multivariate normal distributed:
...
With data shown in @tbl-dat1, we can estimate this model using standard (restricted) maximum likelihood methods as implemented in, for example, the R package lme4 [@batesFittingLinearMixedEffects2015; @rcoreteamLanguageEnvironmentStatistical2023]. We show a conventional summary of this model's estimated parameters in @tbl-lmer.
...
$p^{-} = Pr(\textrm{Valence effect} \le 0) = \Phi(0; \beta_1, \tau_1) = \Phi(0;$ `r mu1[1]`, `r sd1[1]`) = `r p_less_zero`. This number, depicted as a horizontal blue line extending from the CDF to the Y-axis in @fig-ml-pneg B, is the probability that a random slope from this population would take a negative value, or, in other words, the proportion of individuals in the population who are expected to endorse positive words faster than negative words. We illustrate this probability and its construction using the CDF in @fig-ml-pneg.

However, using zero as a critical value might not be sufficiently informative, especially when theory allows specifying a smallest effect size of interest that delimits a region of practical equivalence (ROPE) to zero [@anvariUsingAnchorbasedMethods2021; @lakensEquivalenceTestingPsychological2018; @kruschkeBayesianDataAnalysis2017; @kruschkeDoingBayesianData2014]. In common applications, ROPE is used to statistically infer whether an estimated parameter, such as the effect of valence on logRT for the average person, is practically significant. But we can equally well use a theory-informed region of effect sizes to describe and make inferences about the heterogeneity distribution of this effect in the population. We depict construction of the [-0.1, 0.1] ROPE in @fig-ml-prope.
...
For example, let us imagine that valence effects in the interval [-0.1, 0.1] are practically equivalent to zero---that is, small enough to not matter from, say, a theoretical perspective. We can again use the normal cumulative distribution function to calculate the proportion of individuals in the population whose valence effect falls within this interval or region of practical equivalence (@fig-ml-prope B). First, we choose appropriate values of $\epsilon$, here -0.1 and 0.1 and depicted as blue vertical lines in @fig-ml-prope B. Given these numbers, we then calculate $p^{ROPE} = Pr(-0.1 \le \textrm{Valence effect} \le 0.1) = \Phi(0.1; \beta_1, \tau_1) - \Phi(-0.1; \beta_1, \tau_1)$ = `r p_rope` (depicted as the difference in the horizontal blue lines in @fig-ml-prope B). In words, `r p_rope` of the population is expected to have valence effects that are within the [-0.1, 0.1] interval and therefore practically equivalent to zero. Note that this statement's validity critically depends on the chosen interval.

## Ratio descriptors

The interval and proportion descriptors describe heterogeneity by referring to units of the measurement, such as logRT in the running example. A contrasting or *relative* way to describe heterogeneity is to express it as a ratio of the effect's standard deviation to its mean. Such relative metrics are concise and can be useful especially when the absolute units are difficult to interpret, or when comparing heterogeneity across different populations or experimental conditions (see below). This ratio, expressed simply as the fraction $\frac{\tau_1}{\beta_1}$ is `r ratio` in the current example[^ratio].

[^ratio]: We refer to the absolute value of the ratio throughout to keep it strictly positive for easier interpretation, but omit from the equation for simplicity.
...
For researchers less familiar with Bayesian inference methods than their frequentist counterparts, we recommend readings such as @kruschkeDoingBayesianData2014, @kruschkeBayesianDataAnalysis2017, and @mcelreathStatisticalRethinkingBayesian2020. In addition, established software packages such as brms [@burknerBrmsPackageBayesian2017; @burknerAdvancedBayesianMultilevel2018] and Stan [@standevelopmentteamStanModelingLanguage2023] faciliate these methods' practical applications. We return to the topics of Bayesian inference and estimation in the Discussion section.
...
The output of Bayesian computations is the posterior probability distribution of the model's parameters. However, closed-form solutions are not available for posterior distributions of many important types of statistical models. Therefore, in practice modern Bayesian methods rely on algorithms that yield many random draws from the posterior distribution [@gelmanBayesianDataAnalysis2013; @ravenzwaaijSimpleIntroductionMarkov2016]. These draws can then be used to calculate, summarize, and visualize any desired quantity of the posterior such as means, variances, correlations, proportions above or below zero, and so on. @tbl-samples-1 illustrates this, showing six random draws of the posteriors of $\beta_1$ and $\tau_1$ (rows). We then computed their ratio in the third column, which then represents (draws from) the ratio's posterior distribution and can be summarized, visualized, etc.
...
@tbl-fit-1 shows summaries of Model 1's population-level parameters' ("fixed" parameters in the frequentist nomenclature) posterior distributions. The second and third columns show their means and standard deviations (which correspond to frequentist point estimates and standard errors). Note that because we used brms's default noninformative prior distributions, the posterior summaries are numerically very similar to the maximum likelihood estimates in @tbl-lmer.
...
Armed with the joint Bayesian posterior distribution of all model parameters, we can now return to the heterogeneity distribution of valence effects in the population. We have 4,000 draws from the posterior distribution of the heterogeneity distribution. Within each draw from the posterior distribution, we can perform any calculation we did previously on only the point estimates.

We first recompute the expected heterogeneity distribution from @fig-1 using the posterior mean values of $\beta_1$ and $\tau_1$ in @fig-2 A (thick black curve). We superimposed this normal density curve on heterogeneity distributions calculated from 100 random posterior draws of $\beta_1$ and $\tau_1$ (light gray curves). From these curves we can see that the true distribution of valence effects might well be less or more heterogeneous than is suggested by the expectation (point estimates). We then perform the same exercise on the CDF as well (@fig-2 B).
...
Summarizing a distribution of intervals entails some challenges, however, because an interval is defined by two quantities---the lower and upper bounds. The 95% most plausible lower bounds of $\textit{HI}_{90}$ range between `r p1s$l`, whereas the 95% most credible upper bounds range between `r p1s$u` (marginal histograms in @fig-cdfs-intervals and @fig-intervals B). Thus, to adequately describe an estimated heterogeneity interval, researchers must communicate two separate uncertainty intervals: In words, we estimate that 90% of the population's valence effects range from `r paste(p1s$hi_low, p1s$l)` to `r paste(p1s$hi_high, p1s$u)`.
...
To answer, we calculate $p^- = Pr(\textrm{Valence effect} \le 0) = \Phi(0; \beta_1, \tau_1)$ for each posterior draw of $\beta_1$ and $\tau_1$. @fig-cdfs-pneg shows 100 light grey posterior draws of the CDF with a vertical line representing zero in blue. The y-axis value where the CDF crosses zero on the x-axis indicates the population proportion of negative valence effects ($p^-$). We also show a histogram of all 4,000 posterior draws of that proportion on the y-axis of @fig-cdfs-pneg, with its associated 95%CI. The model predicts the proportion of individuals in the population with negative valence effects to be approximately `r percent(as.numeric(post1_prop$pd), .1)` (posterior mean), but with 95% confidence this value could be as low as `r percent(as.numeric(post1_prop$pd.lower), .1)` or as high as `r percent(as.numeric(post1_prop$pd.upper), .1)`. Stated differently, the model predicts that `r post1_prop$pd_r_n` of individuals in the population would show reversals of the valence effect.
...
We find in @tbl-fit-2 that the average recall performance was worse in the Free recall ($\beta_{m:F}$) than in the cued recall condition ($\beta_{m:C}$; note the parameter estimates refer to probits, not proportions). More importantly, we reproduced @mahVariabilitySubjectsFree2024's finding that participants' memory performance was more heterogeneous in the cued recall task (row 4) than in the free recall task (row 3). We show the relevant estimated quantities and the implied heterogeneity distributions in @fig-2-person.
...
We also depict the heterogeneity distribution's posterior distribution as a PDF and a CDF in @fig-2-person B. Unlike in @fig-2 where we represented random draws of the functions' posteriors as thin lines, @fig-2-person instead aggregates the posteriors to means (dark line) and 95% credibility ribbons (light areas) to reduce overplotting. These figures allow for concise and complementary descriptions of (differences in) heterogeneity in the two tasks. In other words, they allow visually comparing the population distributions of memory performance across the cued and free-recall tasks.

First, we see that the majority of the free recall group's CDF (green) is to the left of zero (50% recall), indicating that the majority of this population is predicted to recall fewer than half of items. This information is described in more detail in the small posterior densities and point-intervals on the left y-axis: The model predicts above-50% performance only for a proportion of `r post2_sum$x[post2_sum$variable=="pd_person_free"]` of the free recall task population. Second, we see that the slope of the cued recall CDF (vivid orange) is less steep and to the right to that of the free recall CDF: The between-person distribution of memory abilities is more dispersed in the cued than in the free recall task. The proportion of individuals in the cued recall task who are predicted to perform above 50% was `r post2_sum$x[post2_sum$variable=="pd_person_cued"]`.

Finally, we turn to the heterogeneity interval (HI). The $\textit{HI}_{90}$'s lower bound in the free recall task is `r post2_sum$x[post2_sum$variable=="hi_low_free"]`, and `r post2_sum$x[post2_sum$variable=="hi_low_cued"]` in the cued recall task (leftmost green and orange densities on bottom x-axis of @fig-2-person B, respectively). While the 5th percentile of the heterogeneity distribution was not credibly different across the two tasks (Cued - Free recall; `r post2_sum$x[post2_sum$variable=="hi_low_difference"]`), the 95th percentiles (rightmost green and orange densities on bottom x-axis of @fig-2-person B, respectively) differed at the 95% credibility level (the Cued recall upper $\textit{HI}_{90}$ limit was `r post2_sum$x[post2_sum$variable=="hi_high_difference"]` probits greater). Studying @fig-2-person B closely makes another implication of the different standard deviations clear: While the average person likely has greater memory performance in the cued recall task, the model predicts that there are also more individuals with very poor performance in the cued recall condition (although this difference was not credibly different from zero).

## Comparing target word heterogeneity across tasks

Between-person heterogeneity is typically the more theoretically important phenomenon for psychologists than differences in model parameters between other randomly sampled study units, such as stimuli. However, examining heterogeneity in other sampled units can be both theoretically and methodologically important [@juddTreatingStimuliRandom2012; @juddExperimentsMoreOne2017]. We therefore turn to our second and third questions regarding potential differences and consistencies in between-target word heterogeneity.
...
Differences in between target-word heterogeneity were similar to those observed for between-person heterogeneity. @fig-2-target A shows that heterogeneity in the probability of a word being correctly recalled was greater when words appeared in the cued recall task (the standard deviation was `r post2_sum$x[post2_sum$variable=="sd_target_difference"]` probits greater in the cued recall task [ratio: `r post2_sum$x[post2_sum$variable=="sd_target_ratio"]`]). Thus, both people and target words exhibit greater memory performance variability in the cued recall than in the free recall task. Moreover, this difference holds even when the exact same units---target words, in this example---are used in the two different tasks.

As was the case for between-person heterogeneity, the model predicts the proportion of words that elicit greater than 50% accurate recall to be greater in the cued recall (`r post2_sum$x[post2_sum$variable=="pd_target_cued"]`) than in the free recall task (`r post2_sum$x[post2_sum$variable=="pd_target_free"]`; difference: `r post2_sum$x[post2_sum$variable=="pd_target_difference"]`). The ratio of the heterogeneity distribution's standard deviation to its mean was again very similar across the two tasks (`r post2_sum$x[post2_sum$variable=="ratio_target_difference"]`).

The design of @mahVariabilitySubjectsFree2024's study and our analysis of the dataset afforded an additional piece of information: Because the same target words were used in the two tasks, we could examine the consistency of target words' heterogeneity across the two tasks (question (3)). There was a clear positive correlation between target words' rates of correct responses across the free and cued recall tasks (bottom panel of @fig-2-target A and C). The posterior mean and 95%CI of this correlation was `r post2_sum$x[post2_sum$variable=="cor_target__taskFree__taskCued"]`.

This correlation's substantive interpretation is that words that are likely better recalled in the free recall task are also likely to be those that are better recalled in the cued recall task. (@bolgerCausalProcessesPsychology2019 found a conceptually similar result regarding valence effects' stability across time but within individuals: Individuals whose valence effect was stronger at Time 1 were also those whose valence effect was likely to be stronger at Time 2, one week later.) The tools presented here might facilitate seeking for theoretically interesting conditions where this consistency is violated.

Our study of Model 2's results might indicate exciting new avenues for this line of inquiry. One explanation for the between-task difference in between-person heterogeneity is that participants might adopt different recall strategies in the two tasks [@mahVariabilitySubjectsFree2024]. Because we find target words, too, to be more heterogeneous in the Cued task (@fig-2-target A), recall strategies and differences therein might further depend on target words. We also observed across both people and target words that the ratio of the between-unit standard deviation to the average effect was nearly identical across the free and cued recall tasks. Finally, given that we operationalized the stability of item difficulties as a correlation across tasks, it might be theoretically important to look for sets of stimuli where this positive correlation did not occur or was particularly strong.

# Discussion

Variability in human behavior is, if not a law, nearly always there: Psychology researchers are usually not surprised to find that a person doesn't respond identically to the same stimulus on two different occasions, or that two different persons don't respond identically to the same stimulus on the same occasion. It is surprising, then, that researchers so rarely directly investigate this hallmark feature of human behavior---variability---beyond modelling it on known predictors.

We think this blind spot results from at least three related issues. One, heterogeneity is so prevalent that theoreticians have paid little attention to it. Consequently so few theories directly consider between-person heterogeneity as their subject matters. Two, and perhaps as a consequence, statistical and analytic approaches are often developed and used with little attention paid to heterogeneity in comparison to modeling features of "the average person", or beyond noting that "people differ from the average". Three, approaches for communicating causal effect heterogeneity in empirical work haven't matured. These three issues---lack of theorizing, lack of attention to adequate modelling approaches, and lack of communication---likely reinforce each other with the end result that heterogeneity is currently treated, at best, as an afterthought.

In this work we described a commonly used existing solution to the second issue---multilevel modelling---and directly addressed the third issue: How to effectively investigate and communicate causal effect heterogeneity. We illustrated the use of practical descriptors of heterogeneity with examples drawn from social and cognitive psychology. Our aim was to incrementally build on the work of @bolgerCausalProcessesPsychology2019 and others---who have described the importance and available methods for examining heterogeneity in causal effects---by describing how it is both critically important and practically feasible to incorporate uncertainty in analyses and descriptions of heterogeneity. Our currently proposed methods incorporate uncertainty into both modelling of and inferences about heterogeneity, and thus allow researchers to go beyond point estimates or model comparison metrics.
...
Second, Schuetze and Hippel [-@schuetze2024, p.3] suggest that "past efforts to identify heterogeneous effects have yielded a disproportionate number of disappointing, uninterpretable, and non-replicable findings", and suggest low power as one potential antecedent. While perhaps an overstatement, one reason for why previous investigations of heterogeneity may have been suboptimal indeed relates to statistical power: By not duly incorporating and reporting on the uncertainty with which heterogeneity is estimated, investigations are more suspect for reporting substantial heterogeneity where it may not truly exist.

## Practical considerations

To aid applied researchers in investigations of heterogeneity, we now turn to immediately applicable practical recommendations for future work interested in examining causal effect heterogeneity.

First, to investigate heterogeneity, a research project must address it directly, rather than as a byproduct of their models or experimental design. That is, because causal effect heterogeneity addresses a different theoretical question---how variable are individuals in the phenomenon of interest---than an average effect, a study's research questions and hypotheses should be defined accordingly [@lundbergWhatYourEstimand2021].

The issue of declaring hypotheses about heterogeneity rather than average effects is perhaps most important when planning a study's sample size. As is standard, researchers should plan their studies and sample sizes for adequate precision to make informative inferences about the parameter(s) focal to their theoretical investigation. Analyses of heterogeneity are no different: When the heterogeneity parameter(s) are of key interest, sample size planning efforts should focus on those, instead of the typical parameters that describe the average persons in the population of interest.

To clarify a study's aims and guide sample size planning, it is useful to select a heterogeneity metric early. While interrelated, targeting a study's inference to sufficient precision regarding e.g. the heterogeneity interval would imply different sample size calculations than targeting it to detect a specific percentage of the population with negative or positive effects. Generally, we believe studies on heterogeneity to require greater sample sizes than those focused on average effects, but leave examinations of this topic to future work.

Second, heterogeneity as we have discussed it here addresses the variability between study units due to factors not included in the model as "fixed" predictors. Thus, the meaning of a study's heterogeneity estimates depend on what predictors are included, and indeed left out, of one's model. The heterogeneity parameters in the models we have considered here quantify model residuals; individuals' deviations from their average in the phenomenon under study. As a consequence, researchers must carefully interpret the meaning of their heterogeneity parameters in light of their models: A model that interacts the phenomenon of interest with a demographic feature, for example, will report heterogeneity beyond that demographic feature, rather than the total possible heterogeneity due to all known and unknown factors.

Third, our focus in this manuscript has been to tightly connect textual, numeric, and visual descriptions of heterogeneity. In many applications, the metrics suggested above provide richer information about heterogeneity than standard model parameters such as the distribution's mean and standard deviation. In Sections 1 and 2 we detailed the calculations underlying these metrics, and then put them to applied use as visualizations in Section 3. We believe these metrics, describing the heterogeneity distribution's location (heterogeneity interval) and the prevalence and relative metrics, and their associated visualizations---whose source code we make available online---can be particularly effective in describing causal effect heterogeneity. For example, if a clear threshold for an effect's prevalence in the population can be established, for example through cost-benefit calculations, researchers ought to emphasize the prevalence metric $p^-$ over interval or relative metrics of heterogeneity.

## Bayesian estimation

We emphasized throughout that the Bayesian approach is well-positioned to answer the needs of researchers interested in heterogeneity. Bayesian methods allow easily carrying uncertainty forward from model parameters to descriptors of heterogeneity and beyond. The resulting metrics' posterior distributions are useful because they not only convey analysts' expectations regarding heterogeneity, but more fully convey their states of knowledge regarding heterogeneity, including degrees of certainty.

However, because many applied researchers are likely to be less familiar with bayesian methods than their frequentist counterparts, here we return to three recurring issues and clarifications in their use.

First, while frequentist estimates consist of a point estimate and its standard error, bayesian estimates are distributions (from which point estimates such as means, and measures of dispersion such as standard error can be calculated). Because these posterior distributions cannot usually be analytically solved, modern bayesian methods instead return a matrix of random draws from the model parameters' joint posterior distribution. This computational detail has at least two important consequences. One, the posterior distributions, and any metrics calculated from them such as means and quantiles for credibility intervals, are approximated from finite samples. This means that unless analysts request very large numbers of posterior draws, numerical results are likely to fluctuate to a very small degree between runs of the estimation algorithm. Two, the matrix of posterior draws is immensely useful, because analysts can use familiar data wrangling techniques to transform the parameters' posterior distributions to posterior distributions of other quantities of interest, such as we have done above. (Our online supplement illustrates these techniques in detail.)

```{r}
x <- post1_prop |>
  mutate(across(where(~ is.numeric(.x)), ~ number(.x, .01))) |>
  str_glue_data("{pd} [{pd.lower}, {pd.upper}]")
```

On a related note, unlike the frequentist confidence interval (CoI), the Bayesian credibility interval (CI) affords assessments of plausibility and confidence. When we reported above that Model 1's $p^-$ posterior summary was `{r} x` (posterior mean and 95%CI), we mean that the 95% of the most likely values of $p^-$ are in the `{r} str_sub(x, 6, -1)` interval, not that the interval constructed in this manner has a 95% chance of capturing the true "fixed" value assumed by frequentist interpretations.

Second, the issue of bayesian analyses' sensitivity to the prior distribution is often mentioned as a somewhat undesirable feature. It is important to note that the prior distribution plays a far larger role in bayesian hypothesis testing, which depends on the prior. When focused on estimation, as we have done here, the impact of prior distributions on the resulting estimates is negligible unless strongly informative priors are used. By default, the software we used [@burknerBrmsPackageBayesian2017] applies noninformative prior distributions that, in practice, only serve to give reasonable starting values for the estimation algorithm. Therefore, in applications when the prior distribution is unknown, leaving it to the software package's defaults is often a very reasonable choice.

Third, bayesian models, just like their frequentist counterparts, should be examined and critiqued. This examination can take the form of examining residual plots, or more generally posterior predictive checks, which can be either visual or numerical. The one additional complexity regarding model checking of bayesian models is that analysts should confirm that the estimation algorithm has "converged"---returned valid values from the posterior distribution. Readers unfamiliar with this topic should peruse introductory literature on bayesian statistics [@kruschkeDoingBayesianData2014; @mcelreathStatisticalRethinkingBayesian2020; @gelmanBayesianDataAnalysis2013].

Finally, while we have here illustrated the use of Bayesian techniques, some methods described here could be implemented with e.g. joint bootstrap methods, but in our view those require additional computational steps---bootstrapping, for one---and might therefore be less practical.

## Limitations

In our example analyses, we have brushed many important topics and modelling decisions under the rug in order to focus on our main topic of heterogeneity. First, in the first example, we analyzed reaction times by simply log-transforming reaction times. More informative analyses of RTs would apply models that make more realistic assumptions about the data generating process underlying reaction time responses, but here we necessarily excluded this complication for reasons of brevity and focus on our main topic. Nevertheless, when more appropriate models are applied (see, for example, @luceResponseTimesTheir1991, @ratcliffDiffusionDecisionModel2008, and @vandekerckhoveHierarchicalDiffusionModels2011), the methods for investigating heterogeneity presented here apply seamlessly.

Second, throughout we have discussed heterogeneity of *causal* effects. A necessary first step in any such endeavor is to appropriately identify the causal effect whose heterogeneity one is interested in by, for example, randomization and experimental control. It is important for readers to carefully consider, and be transparent about, whether their study and analytic design allow causal inferences, and by extension heterogeneity of causal effects. See, for example, @lundbergWhatYourEstimand2021 and @rohrerThinkingClearlyCorrelations2018.

Third, our exposition and interpretation of heterogeneity relies on a critical assumption in line with standard practices in multilevel and generalized linear mixed modelling; that of normality of the unit-level (person, item, etc) parameters. Assuming that random effects are normally distributed is a computationally and conceptually useful fiction, and we recognize that it is unlikely to hold exactly in real psychological phenomena. Haaf, Rouder, and colleagues have explored alternatives to continuous normal distributions of random effects [e.g., @haafDevelopingConstraintBayesian2017; @haafDonAccountingVariability2019].

## Conclusion

We believe that psychology, broadly speaking, is ripe for incorporating effect heterogeneity into substantive theories [@bolgerCausalProcessesPsychology2019]. To do so, descriptions of heterogeneity must include measures of uncertainty. We hope that the conceptual, computational, and graphical tools that we have discussed here prove useful to researchers interested in better understanding heterogeneity in psychological phenomena.
